...
my @shortNS = @namespace;
if ($shortNS[0] eq 'afl') {
    shift @shortNS;
}

my $testClassName = 'Test'.join('', map {ucfirst} @shortNS).$className;
my $testFileName = lc(join('_', 't', @shortNS, $className)).'.cpp';
my $testHeaderName = lc(join('_', 't', @shortNS)).'.hpp';
my $testGuardName = uc(join('_', 'U', 'T', @shortNS, 'HPP'));
...
sub generateHeaderFile {
    ...
    if (open HEADER, "< u/$testHeaderName") {
        ...
        @header = ("/**\n",
                   ...
                   "  */\n",
                   "#ifndef AFL_$testGuardName\n",
                   "#define AFL_$testGuardName\n",
                   "\n",
