...
{
    getHandler().callVoid(Segment().pushBack("APPEND").pushBack(getName()).pushBack(text));
}
...
{
    return getHandler().callString(Segment().pushBack("GET").pushBack(getName()));
}
...
afl::net::redis::StringKey::substr(std::size_t start, std::size_t length) const
{
    if (start < MAX_SIZE && length > 0) {
        ...
        return getHandler().callString(Segment().pushBack("GETRANGE").pushBack(getName())
                                       .pushBack(static_cast<int32_t>(start))
                                       .pushBack(static_cast<int32_t>(start+length-1)));
    } else {
        return String_t();
    }
}
...
{
    return getHandler().callString(Segment().pushBack("GETSET").pushBack(getName()).pushBack(newValue));
}
...
{
    int32_t n = getHandler().callInt(Segment().pushBack("STRLEN").pushBack(getName()));
