/**
  *  \file u/t_io_archive_tarreader.cpp
...
void
TestIoArchiveTarReader::testListCompressed()
{
    ...
    TS_ASSERT(e.get() != 0);
    TS_ASSERT_EQUALS(e->getTitle(), "a.txt");
    TS_ASSERT_EQUALS(e->getFileType(), DirectoryEntry::tFile);
    TS_ASSERT_EQUALS(e->getFlags(), DirectoryEntry::FileFlags_t());

    TS_ASSERT(iter->getNextElement(e));
    TS_ASSERT(e.get() != 0);
    TS_ASSERT_EQUALS(e->getTitle(), "b.txt");
    TS_ASSERT_EQUALS(e->getFileType(), DirectoryEntry::tFile);
    TS_ASSERT_EQUALS(e->getFlags(), DirectoryEntry::FileFlags_t());

    TS_ASSERT(iter->getNextElement(e));
    TS_ASSERT(e.get() != 0);
    TS_ASSERT_EQUALS(e->getTitle(), "c.sh");
    TS_ASSERT_EQUALS(e->getFileType(), DirectoryEntry::tFile);
    TS_ASSERT_EQUALS(e->getFlags(), DirectoryEntry::FileFlags_t(DirectoryEntry::Executable));
...
/** Test listing a compressed file, with paths.
    The call must produce the known members in correct order. */
void
TestIoArchiveTarReader::testListWithPath()
{
    // Skip test if zlib is not available
    if (!afl::io::InflateTransform::isAvailable()) {
        return;
    ...
    TS_ASSERT(iter->getNextElement(e));
    TS_ASSERT(e.get() != 0);
    TS_ASSERT_EQUALS(e->getTitle(), "a.txt");                           // note "./" has been stripped

    TS_ASSERT(iter->getNextElement(e));
    TS_ASSERT(e.get() != 0);
    TS_ASSERT_EQUALS(e->getTitle(), "b/b.txt");
...
void
TestIoArchiveTarReader::testListUncompressed()
{
    // Setup
...
void
TestIoArchiveTarReader::testReadCompressed()
{
    // Skip test if zlib is not available
    if (!afl::io::InflateTransform::isAvailable()) {
        return;
    ...
    // Nonexistant entry
    TS_ASSERT_THROWS(testee->getDirectoryEntryByName("blub"), afl::except::FileProblemException);
}
...
void
TestIoArchiveTarReader::testReadUncompressed()
{
    // Setup
    Ref<afl::io::archive::TarReader> testee = afl::io::archive::TarReader::open(*new afl::io::ConstMemoryStream(UNCOMPRESSED_FILE), 0);
...
void
TestIoArchiveTarReader::testNameCompressed()
{
    // Skip test if zlib is not available
    if (!afl::io::InflateTransform::isAvailable()) {
        return;
