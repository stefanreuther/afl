/**
  *  \file u/t_io_inflatedatasink.cpp
...
void
TestIoInflateDataSink::testGood()
{
    if (!afl::io::InflateDataSink::isAvailable()) {
        // Test how it refuses being constructed.
        afl::io::InternalSink t;
        TS_ASSERT_THROWS(new afl::io::InflateDataSink(t, afl::io::InflateDataSink::Gzip),
    ...
    for (size_t i = 0; i < countof(blockSizes); ++i) {
        ...
        // Check result
        TS_ASSERT_EQUALS(t.getContent().size(), DECODED_GZIP_STREAM_size);
        TS_ASSERT_SAME_DATA(t.getContent().unsafeData(), DECODED_GZIP_STREAM, DECODED_GZIP_STREAM_size);
    }
}
